...
const verticalCard = ({ category, heading }) => {
  ...
  return (
    <div className="container mx-auto px-3 my-2 relative">
      ...
      <div className="flex items-center gap-3 md:gap-4 overflow-scroll scrollbar-none transition-all" ref={scrollEement}>
        <button
        ...
        </button>
        {
            loading ?(
                ...
                data.map((product, index) => {
                    return (
                      <Link to={"product/"+product?._id} className=" w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] bg-white rounded-md" key={index}>
